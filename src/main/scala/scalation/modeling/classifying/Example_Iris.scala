 ...
 *      val x = xy.nor (?, 5)         // columns 1, 2, 3, 4
...
/*
    ...
    analyze (nm)
    println (nm.summary (nm.parameter))
    ...
    analyzer (nbr)
    println (nbr.summary (nbr.parameter))
    ...
    analyzer (lda)
    println (lda.summary (lda.parameter))

    banner ("KNN_Classifier")
    val knn = new KNN_Classifier (xx, yy)
    analyzer (knn)
    println (knn.summary (knn.parameter))

    banner ("DecisionTreeC45")
    val dtc = new DecisionTreeC45 (xx, yy)
    analyzer (dtc)
    println (dtc.summary (dtc.parameter))
    ...
    analyzer (svm)
    println (svm.summary (svm.parameter))
 */
